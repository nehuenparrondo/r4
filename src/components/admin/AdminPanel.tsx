...
import type { CollectionEntity, EntityName } from '../../types/portfolio'

const tabs: ReadonlyArray<readonly [EntityName, string]> = [
  ['skills', 'Habilidades'],
...
export function AdminPanel({ open, onClose }: AdminPanelProps) {
  const [activeTab, setActiveTab] = useState<EntityName>('skills')
  ...
  return (
    <div className="admin-backdrop" role="presentation" onMouseDown={(event: MouseEvent<HTMLDivElement>) => event.target === event.currentTarget && onClose()}>
        ...
        {!unlocked ? (
          ...
          <>
            ...
            <div className="admin-content">
              {activeTab === 'contact' ? <ContactForm secret={secret} /> : <EntityManager entity={activeTab as CollectionEntity} secret={secret} />}
            </div>
          </>
        )}
      </aside>
